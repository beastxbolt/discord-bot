...
class voteskip(commands.Cog):
    ...
    async def voteskip(self, ctx):
        ...
        async def votePassed(msg, yesVotes, requiredYesVotes):
            embed = discord.Embed(title=f"{tick} Vote Passed! Skipping...", description=f"**{yesVotes}/{requiredYesVotes}** Voted YES", colour=0x00ff00,timestamp=ctx.message.created_at)
            await msg.clear_reactions()
            await msg.edit(embed=embed)
            return

        async def voteFailed(msg, noVotes, requiredYesVotes):
            embed2 = discord.Embed(title=f"{cross} Vote Failed!", description=f"**{noVotes}/{requiredYesVotes}** Voted NO", colour=0xff0000,timestamp=ctx.message.created_at)
            await msg.clear_reactions()
                    ...
                    if ctx.author.voice is not None and ctx.author.voice.channel == ctx.voice_client.channel:
                        if ctx.voice_client.is_playing() or ctx.voice_client.is_playing() and ctx.voice_client.is_paused() is False or ctx.voice_client.is_playing() and ctx.voice_client.is_paused():
                            if self.bot.voice_client_attributes[str(ctx.guild.id)]['block_skip_and_stop'] is False:
                                ...
                                if len(users) == 1:
                                    votePassedEmbed = discord.Embed(title=f"{tick} Vote Passed! Skipping...", colour=0x00ff00,timestamp=ctx.message.created_at)
                                ...
                                if len(users) == 2:
                                    ...
                                    try:
                                        ...
                                        embed6 = discord.Embed(title=f"{cross} Vote Failed!", description=f"**Not Enough Votes!**", colour=0xff0000,timestamp=ctx.message.created_at)
                                        await msg.clear_reactions()
                                ...
                                if len(users) >= 3:
                                    ...
                                    try:
                                        ...
                                        embed10 = discord.Embed(title=f"{cross} Vote Failed!", description=f"**Not Enough Votes!**", colour=0xff0000,timestamp=ctx.message.created_at)
                                        await msg.clear_reactions()
                                        await msg.edit(embed=embed10)
                                        return
                            else:
                                embed11 = discord.Embed(title="Cannot Skip While Adding Playlist In Queue!",colour=0xff0000,timestamp=ctx.message.created_at)
                                await ctx.send(embed=embed11)
                                return
                        else:
                            embed12 = discord.Embed(title="Player Is Not Playing!",colour=0xff0000,timestamp=ctx.message.created_at)
                            await ctx.send(embed=embed12)
                            return
                    else:
                        embed13 = discord.Embed(title="You Must Be In The Same Voice Channel As The Bot To Use This Command!",colour=0xff0000,timestamp=ctx.message.created_at)
                        await ctx.send(embed=embed13)
                        return
                else:
                    embed14 = discord.Embed(title="I'm Not Connected To A Voice Channel!",colour=0xff0000,timestamp=ctx.message.created_at)
                    await ctx.send(embed=embed14)
                    return
            else:
                embed15 = discord.Embed(title="You Must Be In A Voice Channel To Use This Command!",colour=0xff0000,timestamp=ctx.message.created_at)
