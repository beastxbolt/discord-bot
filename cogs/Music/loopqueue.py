    ...
    async def loopqueue(self, ctx):
        ...
        async def loopqueueFunc():
            if self.bot.voice_client_attributes[str(ctx.guild.id)]['loop_queue'] is False:
                ...
                embed = discord.Embed(title=":repeat: Loop Queue Enabled :repeat:",colour=0x0000ff,timestamp=ctx.message.created_at)
                await ctx.send(embed=embed)
                return
            else:
                var = {'loop_queue': False}
                self.bot.voice_client_attributes[str(ctx.guild.id)].update(var)
                embed2 = discord.Embed(title=":repeat: Loop Queue Disabled :repeat:",colour=0x0000ff,timestamp=ctx.message.created_at)
