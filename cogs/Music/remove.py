    ...
    async def remove(self, ctx, index : int = None):
        ...
        async def removeFunc():
            try:
                ...
                embed2 = discord.Embed(description=f"**:white_check_mark: Removed -** {title}",colour=0x00ff00,timestamp=ctx.message.created_at)
                await ctx.send(embed=embed2)
                self.bot.music[str(ctx.guild.id)].pop(title)
                return
            except IndexError:
                embed3 = discord.Embed(title="The Song You Tried To Remove Does Not Exist!",colour=0xff0000,timestamp=ctx.message.created_at)
                await ctx.send(embed=embed3)
                return

        if index is None:
            embed = discord.Embed(title=f"This Command Is Used Like This: `{prefix(self.bot, ctx)[0][0]}remove [position]`",colour=0xff0000,timestamp=ctx.message.created_at)
            await ctx.send(embed=embed)
            return

        if index <= 0:
            embed = discord.Embed(title="The Song You Tried To Remove Does Not Exist!",colour=0xff0000,timestamp=ctx.message.created_at)
            await ctx.send(embed=embed)
            return
        ...
        try:
            if ctx.author.voice is not None:
                ...
                embed6 = discord.Embed(title="You Must Be In A Voice Channel To Use This Command!",colour=0xff0000,timestamp=ctx.message.created_at)
