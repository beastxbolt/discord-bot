...
class resume(commands.Cog):
    ...
    async def resume(self,ctx):
        ...
        async def resumeFunc():
            ...
            embed = discord.Embed(title="Resumed!",colour=0x0000ff,timestamp=ctx.message.created_at)
                ...
                if ctx.voice_client is not None:
                    if ctx.author.voice is not None and ctx.author.voice.channel == ctx.voice_client.channel:
                        if not ctx.voice_client.is_playing() and ctx.voice_client.is_paused() is True:
                            ...
                            embed2 = discord.Embed(title="Player Is Not Paused!",colour=0xff0000,timestamp=ctx.message.created_at)
                            await ctx.send(embed=embed2)
                            return
                    else:
